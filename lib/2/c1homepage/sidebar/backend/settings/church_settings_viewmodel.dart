...
class ChurchSettingsViewModel extends ChangeNotifier {
  void onNavigate(AdminView view, {AuthenticatorFlow? flow, String? newValue}) {
    _onNavigate(view, flow: flow, newValue: newValue);
  }
  ...
  ChurchSettingsViewModel({
    ...
    required Function(AdminView, {AuthenticatorFlow? flow, String? newValue})
        onNavigate,
  ...
  void submitChangeEmail() {
    if (emailFormKey.currentState?.validate() ?? false) {
      _onNavigate(AdminView.authenticator,
          flow: AuthenticatorFlow.email, newValue: newEmailController.text);
    }
  }
  ...
  void submitChangePassword() {
    if (passwordFormKey.currentState?.validate() ?? false) {
      ...
      _onNavigate(AdminView.authenticator,
          ...
          newValue: newPasswordController.text);
    }
  }
  ...
  void submitChangePhone() {
    if (phoneFormKey.currentState?.validate() ?? false) {
      _onNavigate(AdminView.authenticator,
          flow: AuthenticatorFlow.phone, newValue: newPhoneController.text);
    }
  }
