// lib/sidebar/frontend/settings/CHURCH_NOTIFICATIONS_SETTING.dart

import 'package:flutter/material.dart';
import 'package:flutter/cupertino.dart';
import 'package:provider/provider.dart';
import '../../backend/models/admin_models.dart'; // Import AdminView
import '../../backend/settings/church_settings_viewmodel.dart'; // Import the new ViewModel

class ChurchNotificationSetting extends StatefulWidget {
  final Function(AdminView) onNavigate;

  const ChurchNotificationSetting({
    super.key,
    required this.onNavigate,
  });

  @override
  State<ChurchNotificationSetting> createState() =>
      _ChurchNotificationSettingState();
}

class _ChurchNotificationSettingState extends State<ChurchNotificationSetting>
  ...
  void didChangeDependencies() {
    ...
    // Initialize animation states based on ViewModel's current values
    final viewModel = Provider.of<ChurchSettingsViewModel>(context, listen: false);
    if (viewModel.pushNotificationsEnabled) {
      _pushAnimationController.value = 1.0;
    } else {
      _pushAnimationController.value = 0.0;
    }
    if (viewModel.emailNotificationsEnabled) {
      _emailAnimationController.value = 1.0;
    } else {
      _emailAnimationController.value = 0.0;
    }
  }
  ...
  Widget build(BuildContext context) {
    return ChangeNotifierProvider(
      create: (context) => ChurchSettingsViewModel(
        adminData: Provider.of<ChurchSettingsViewModel>(context, listen: false)._adminData, // Pass existing adminData
